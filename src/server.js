...
const clusterEngine = new ClusterEngine(balancer, inboundManager);
...
app.post('/api/cluster/sync', async (req, res) => {
  const reqToken = req.headers['x-cluster-token'] || (req.body ? req.body.clusterKey : '');
  const originUrl = req.headers['x-cluster-origin'] || (req.body ? req.body.originUrl : '');

  // 1. Bi-directional Mirroring: Auto-register caller panel on recipient node so it displays on child panel as well!
  if (originUrl && typeof originUrl === 'string') {
    clusterEngine.addPeer(originUrl, reqToken);
  }

  const { mode, nodes, inbounds, countryOrder } = req.body || {};

  if (mode) balancer.setMode(mode);
  if (countryOrder && Array.isArray(countryOrder)) balancer.countryOrder = countryOrder;

  // 2. Smart Delta Merge VPN Nodes
  if (nodes && Array.isArray(nodes)) {
    balancer.addNodes(nodes);
  }

  // 3. Smart Delta Merge HTTP/SOCKS Inbound Proxy Ports
  if (inbounds && Array.isArray(inbounds)) {
    const localInboundPorts = new Set(inboundManager.getInbounds().map(i => i.port));
    for (const remoteInb of inbounds) {
      if (!localInboundPorts.has(remoteInb.port)) {
        try {
          await inboundManager.addInbound({
            type: remoteInb.type,
            port: remoteInb.port,
